...
let fullNames = [];
//loop over the data- find firstname and lastname and push to new array
...
//loop over the data- find firstname and capitalize and push to new array
...
let largeSize = runners.filter(function(size) {
	if (size.shirt_size === "L") {
		return runnersLargeSizeShirt.push(size);
	}
});
...
// Problem 1
//loop over data for emails for subscription
let emails = [];
runners.forEach(function(names) {
	return emails.push(`${names.first_name} ${names.email}`);
});
console.log(emails);

// Problem 2
//loop over the data- uppercase full names of all runners
let fullNamesUpperCase = [];
runners.map(function(item) {
	fullNamesUpperCase.push(`${item.first_name.toUpperCase()} ${item.last_name.toUpperCase()}`);
});
console.log(fullNamesUpperCase);

// Problem 3
// Disqualify runners who do not donate more than $10
let disqualifyRunners = [];
console.log(`The list of runners who have been disqualified for donating less than $10:`);
let donationSize = runners.filter(function(number) {
	if (number.donation <= 10) {
		console.log(`${number.first_name} ${number.last_name} has been disqualified! ${number.donation}`);
		disqualifyRunners.push(number);
	}
});
